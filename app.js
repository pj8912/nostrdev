...
function publishNote() {
    ...
    console.log(document.querySelector('#note').value.trim())
    console.log(document.querySelector('#private_key').value.trim())


    fetch('publish_note.php', {
        method: "POST",
        body: JSON.stringify({

            request: "upload_post",
            data: document.querySelector('#note').value.trim(),
            privkey: document.querySelector('#private_key').value.trim()
        })
    })
        .then(res => res.json())
        .then(response => {
            if (response.status == 1) {
                ...
                console.log(response.event_message)
               
               
                // note uploaded status, and note event string
                // document.querySelector('#status_message').innerHTML = '<span style="text:green">Note Uploaded!!</span>';
                // setTimeout(function () {
                //     document.getElementById("status_message").innerHTML = '';
                // }, 3000);
                ...
                // // eventstring
                // var eveString = document.createElement('div')
                // // eeveString.style.padding = '10px;'
                // eveString.setAttribute('class', 'mb-2 p-3')

                // eveString.innerHTML = response.event_message
                // document.querySelector('#estring').innerHTML = eveString
            }
            // else if (response.status == 0) {
            //     document.querySelector('#status_message').innerHTML = '<span style="text:red">Note Upload Failed</span>';
            //     setTimeout(function () {
            //         document.getElementById("status_message").innerHTML = '';
            //     }, 3000);
            // }
    ...
    document.querySelector('#private_key').value = '';
}